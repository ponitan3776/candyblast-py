...
.title{ 
    ...
    color:var(--gold);                             /* ★保険の色（背景が効かなかった時用） */
    background:linear-gradient(90deg, var(--gold), var(--pink)); 
    -webkit-background-clip:text; background-clip:text;
    -webkit-text-fill-color:transparent;           /* ★color ではなくこちらを使う */
    cursor: pointer;
    padding: 2px 6px;
    border-radius: 8px;
...
.title:hover{ filter: brightness(1.25); }
  ...
  /* パン(流れる): 多色グラデーションを大きめのbackground-sizeで敷き、position をゆっくり往復させる */
  body[data-living-anim="pan"] .cell.filled,
  body[data-living-anim="pan"] .tetris-cell.filled{
    background-size:280% 280% !important;
    animation:livingSkinPan 5s ease-in-out infinite;
  }
  @keyframes livingSkinPan{
    0%{ background-position:0% 50%; }
    50%{ background-position:100% 50%; }
    100%{ background-position:0% 50%; }
  }
  /* パルス(鼓動): 明るさとわずかな拡大縮小を心拍のようなリズムで繰り返す */
  body[data-living-anim="pulse"] .cell.filled,
  body[data-living-anim="pulse"] .tetris-cell.filled{
    animation:livingSkinPulse 1.15s cubic-bezier(0.4,0,0.2,1) infinite;
  }
  @keyframes livingSkinPulse{
    0%, 100%{ filter:brightness(1) saturate(1); transform:scale(1); }
    18%{ filter:brightness(1.4) saturate(1.5); transform:scale(1.05); }
    32%{ filter:brightness(1) saturate(1); transform:scale(1); }
    46%{ filter:brightness(1.25) saturate(1.35); transform:scale(1.03); }
    62%{ filter:brightness(1) saturate(1); transform:scale(1); }
  }
  /* 色相回転: 色そのものを渦のように連続的に回し続ける */
  body[data-living-anim="hue"] .cell.filled,
  body[data-living-anim="hue"] .tetris-cell.filled{
    animation:livingSkinHue 4s linear infinite;
  }
  @keyframes livingSkinHue{
...
.tetris-controls{
 ...
 /* 画面の高さが低いときは自動でボードを縮める(縦向き含む) */

/* 画面の高さが低いほど、ボードを自動で縮める */
  @media (max-height: 820px){
    #tetrisBoard{ height: min(50dvh, 420px); }
  }
  @media (max-height: 700px){
    #tetrisBoard{ height: min(46dvh, 360px); }
  }
  @media (max-height: 600px){
    #tetrisBoard{ height: min(42dvh, 300px); }
  }

  /* iPad横画面など、横長で高さが低いビューポートは横並びレイアウトに切り替え */
  @media (orientation: landscape) and (max-height: 700px){
    ...
    #tetrisBoard{ height: min(82dvh, 460px); width:auto; }
    .tetris-controls{ display:flex; flex-direction:column; width:150px; gap:8px; margin-top:0; flex-shrink: 0; }
...
.tetris-vs-label.you{ color:var(--mint); }
.tetris-vs-label.ai{ color:var(--coral); }
.tetris-vs-score{
  font-family:'Baloo 2', sans-serif;
  font-size:13px;
  font-weight:700;
...
#tetrisAiBoard{
  opacity:0.92;
  box-shadow: 0 10px 30px rgba(0,0,0,0.35), inset 0 0 0 2px rgba(255,107,107,0.25);
}
#tetrisBoard{
  box-shadow: 0 10px 30px rgba(0,0,0,0.35), inset 0 0 0 2px rgba(61,220,151,0.25);
}
...
.tetris-hud-bottom .tetris-help-btn{
  width:120px;
}

/* 縦が狭い画面用：盤面をさらに縮小 */
@media (max-height: 720px){
  #tetrisBoard, #tetrisAiBoard{ height: min(42dvh, 340px); }
}
@media (max-height: 600px){
  #tetrisBoard, #tetrisAiBoard{ height: min(36dvh, 280px); }
}

/* 縦向きスマホ（幅が狭い）でも2枚並ぶように微調整 */
@media (max-width: 420px){
  .tetris-vs-layout{ gap:4px; }
  .tetris-hud{ min-width:56px; gap:4px; }
  #tetrisHoldBox, #tetrisNext{ width:56px; height:56px; padding:4px; }
  .vs-badge{ font-size:14px; }
}

/* ===================== テトリス ソロモード ===================== */
/* ソロモード時はAI側とVSバッジを隠す */
#tetris-wrap.solo-mode #tetrisAiSide,
#tetris-wrap.solo-mode .vs-badge {
  display: none;
}
#tetris-wrap.solo-mode .tetris-vs-layout {
  justify-content: center;
}
/* ソロモード時はプレイヤー側の盤面を大きく */
#tetris-wrap.solo-mode #tetrisBoard {
  height: min(60dvh, 480px);
}
@media (max-height: 720px){
  #tetris-wrap.solo-mode #tetrisBoard{ height: min(52dvh, 420px); }
}
@media (max-height: 600px){
  #tetris-wrap.solo-mode #tetrisBoard{ height: min(44dvh, 340px); }
}
